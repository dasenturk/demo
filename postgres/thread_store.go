...
func (s *ThreadStore) Thread(id uuid.UUID) (petsy.Thread, error) {
	var t petsy.Thread
	if err := s.Get(&t, `SELECT * FROM threads WHERE id = $1`, id); err != nil {
		return petsy.Thread{}, fmt.Errorf("error getting thread: %w", err)
	}
	return t, nil
}

func (s *ThreadStore) Threads() ([]petsy.Thread, error) {
	var tt []petsy.Thread
	if err := s.Select(&tt, `SELECT * FROM threads`); err != nil {
		return []petsy.Thread{}, fmt.Errorf("error getting threads: %w", err)
	}
	return tt, nil
}

func (s *ThreadStore) CreateThread(t *petsy.Thread) error {
...
func (s *ThreadStore) UpdateThread(t *petsy.Thread) error {
