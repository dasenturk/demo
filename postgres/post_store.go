...
func (s *PostStore) Post(id uuid.UUID) (petsy.Post, error) {
	var p petsy.Post
	if err := s.Get(&p, `SELECT * FROM posts WHERE id = $1`, id); err != nil {
		return petsy.Post{}, fmt.Errorf("error getting post: %w", err)
	}
	return p, nil
}

func (s *PostStore) PostsByThread(threadID uuid.UUID) ([]petsy.Post, error) {
	var pp []petsy.Post
	var query = `
		SELECT
			posts.*,
	...
	if err := s.Select(&pp, query, threadID); err != nil {
		return []petsy.Post{}, fmt.Errorf("error getting posts: %w", err)
	}
	return pp, nil
}

func (s *PostStore) Posts() ([]petsy.Post, error) {
	var pp []petsy.Post
	var query = `
		SELECT
			posts.*,
...
func (s *PostStore) CreatePost(p *petsy.Post) error {
...
func (s *PostStore) UpdatePost(p *petsy.Post) error {
